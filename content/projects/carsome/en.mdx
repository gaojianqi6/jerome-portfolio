...
This separation kept warehouse actions responsive and made dependency failures easier to isolate. It also made the operational risks explicit: cross-boundary correlation, safe replay of failed jobs, idempotency contracts, and queue-age visibility.
...
The underlying design remains useful: one clear inventory domain, thin transport layers, asynchronous side effects, and clients built around stable contracts. The main operational considerations are end-to-end telemetry, separate liveness and readiness, compound indexes verified against real query shapes, and controlled replay of failed jobs with clear ownership.
